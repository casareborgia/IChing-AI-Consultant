...
# 초점·체용 규칙의 판본. 리포트가 "어느 규칙으로 고른 근거인가"를 기록할 때 쓴다.
# **규칙을 바꾸면 이 문자열도 바꾼다** — 저장된 리포트가 어느 판정으로 만들어졌는지
# 나중에 구분할 수 없으면, 규칙을 고친 뒤 옛 리포트를 재해석하게 된다.
FOCUS_RULE_VERSION = "zhuxi-gobyeonjeom-2026-08-30"

# ---------------------------------------------------------------------------
# 체용(體用) 보완 규칙 상수
#
# 《춘추좌전》 점례를 정리한 상수역 계열 규칙이다. 정전(의리역)에는 없다 —
# 전거와 확인 상태는 `data/PROVENANCE.md` 「해석 규칙의 전거」에 적었다.
#
# 지괘가 아래 9괘이면 본괘(體) 위주로 해석한다 — 비(否/12), 점(漸/53), 려(旅/56),
# 함(咸/31), 미제(未濟/64), 곤(困/47), 고(蠱/18), 정(井/48), 항(恒/32)
...
def _attach_body_use(
    ...
    **두 규칙은 같은 층이 아니다.** 초점 규칙(고변점)이 *어느 근거를 꺼낼지*를
    ...
    이 구분이 필요한 이유는 두 규칙이 실제로 어긋나기 때문이다 — 4,032개 조합 중
    387개에서 서로 다른 괘를 가리킨다(효사 초점과 체용이 갈리는 자리다. 3변효는
    초점이 BOTH라 어긋나지 않는다). 예전에는 두 문구가 대등하게 나란히 붙어,
    모델과 화면이 "본괘의 괘사를 주 해석으로 삼습니다"와 "지괘를 중심으로 무게를
    둡니다"를 동시에 받았다. 어느 쪽을 따라야 하는지 아무도 말해주지 않았고,
    체용이 가리키는 괘의 괘사는 근거 목록에 없기까지 했다.
...
def _focus_by_changing_count(
    ...
    if count == 0:
        ...
        # 《역학계몽》 고변점: 三爻變則占本卦及之卦之彖辭 而以本卦爲主.
        # 본괘·지괘 괘사를 함께 보되 본괘를 위주로 한다.
        #
        # 한때 초효 포함 여부로 본괘/지괘를 갈랐다가 되돌렸다. 되돌린 이유는
        # **체용(體用) 규칙과 역할이 겹치기 때문**이다. 체용은 애초에 계몽의
        # 표준 공식만으로 《춘추좌전》 점례가 설명되지 않는 자리를 메우려고
        # 후대가 만든 보완 규칙이다 — 본괘냐 지괘냐를 가리는 일이 그쪽 몫이다.
        # 초점에서 한 번 가르고 체용이 또 가르면 두 규칙이 같은 결정을 겹쳐
        # 내리게 되고, 실제로 동효 3개 자리에서 충돌이 가장 많았다(180건).
        #
        # BOTH로 두면 근거에 두 괘사가 다 실리므로, 체용이 어느 쪽을 가리켜도
        # 댈 재료가 있다. 전거는 `data/PROVENANCE.md`에 적었다.
        return FocusRuleResult(
            focus_type=FocusType.BOTH_JUDGMENTS,
            target_hexagram_type="BOTH",
            target_line_numbers=[],
            description_ko=(
                "3개의 변효가 있으므로 본괘 괘사와 지괘 괘사를 함께 참작하되, "
                "본괘 괘사를 위주로 합니다."
            ),
        )
    elif count == 4:
