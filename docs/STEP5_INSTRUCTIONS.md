...
## 2-1. 배포 리팩터가 깨뜨리면 안 되는 것 (2026-08-16 추가)

5단계 산출물을 Cloud Run·Firestore·ADK 쪽으로 다시 짤 예정이라면, 아래 넷은
구조가 바뀌어도 살아남아야 한다. 전부 실제로 한 번씩 겪은 자리다.

**① 안전 스크리닝은 어떤 경로에서도 건너뛰지 않는다.** "가벼운 인사는 지름길로"
같은 동적 라우팅을 입구에 얹는 설계가 흔히 제안되는데, 스크리너 **앞**에 두면
안 된다. 위기는 첫 발화가 아니라 대화가 깊어지며 드러나고, 인사처럼 보이는
발화도 예외가 아니다. 라우팅은 스크리너 **뒤**에 놓는다. 이 제약은
`test_모든_턴이_예외없이_스크리닝을_먼저_거친다`가 지킨다 — 지름길을 넣으면
그 테스트가 깨진다.

**② 프롬프트의 출처는 하나다.** 지금은 `prompts/*.md`를 채점기와 에이전트가
같은 파일에서 읽는다. 운영 규칙을 Firestore 같은 DB로 옮겨 무중단으로 고치겠다면
**채점기도 같은 곳을 읽어야 한다.** 갈라지면 채점한 프롬프트와 서비스되는
프롬프트가 달라져 점수가 거짓말이 된다. 이 저장소는 안전 문구가 파일과 갈라진
전례가 이미 있다.

**③ 가짜 LLM을 주입할 수 있어야 한다.** 지금 75개 테스트가 네트워크 없이 도는
근거이고, 안전 동작(래치·차단·라벨 비노출)이 전부 그 위에 서 있다. 프레임워크를
갈아타더라도 클라이언트를 밖에서 넣는 구멍은 남긴다. 이게 막히면 안전 보장을
테스트로 붙들 수 없다.

**④ 판정은 서버 밖으로 나가지 않는다.** `category`·`signals`·`reason`은 로그와
DB에만 남는다. 상태를 외부 저장소로 옮기더라도 사용자 응답에 실리는 필드와
내부 필드의 경계(`TurnResult`)는 유지한다. SaMD 판정선과 닿는 자리다.

### 상태와 저장소에 관하여

이 앱은 **이미 무상태다.** 인메모리 가변 상태가 없고 세션·턴·저널이 전부
Postgres에 있다. "서버가 꺼지면 대화가 증발한다"는 문제는 해당되지 않는다.
세션을 Firestore로 옮기는 것은 선택이지 필수가 아니며, 옮기면 RAG(pgvector)와
저장소가 둘이 되고 재삼독 판단에 쓰는 "과거 세션 + 저널 요약" 조회가 저장소를
넘나들게 된다. 얻는 것을 먼저 정하고 갈 것.

에이전트끼리 텍스트를 주고받지도 않는다. 파이프라인이 순서대로 부르고 타입이
정해진 객체를 넘긴다. 그래서 "본문 대신 참조 ID를 주고받아 토큰을 줄인다"는
설계는 이 구조에 걸릴 자리가 거의 없다. 줄일 데가 있다면 상담 프롬프트에 매 턴
근거가 다시 실리는 그 한 곳이고, 그건 참조 태그가 아니라 캐싱 문제다.
기준선은 `scripts/measure_session.py`가 낸다 — 줄었는지 판정하려면 줄이기 전
숫자가 있어야 한다.
