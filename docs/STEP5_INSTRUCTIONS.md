...
2. 이 문서 — 5단계에서 무엇을, 어떤 계약으로, 어떤 순서로 만들지.
3. `docs/PROGRESS_AND_HANDOFF_STEP5.md` — **낡았다.** 2026-08-14 기준이라 저본
   이관(4-1) 이전 사실이 남아 있다(`hexagrams.json`, 청크 1,751건, 커밋 f372186).
   지금은 참고하지 말고, 5단계를 마칠 때 이 지시서 기준으로 갱신하거나 지운다.
