...
**대조군은 천장이지 상수가 아니다.** 처방이 상투구 자체를 줄이면 같은 괘 반복 쌍도
덜 닮게 되어 대조군이 함께 내려간다. 그러면 "대조군과의 차이"는 오히려 작아진다.
차이 한 줄만 보고 판단하지 말 것 — 다른괘 공유와 전역 상투구를 먼저 본다.
...
def 대조(왼_이름: str, 왼: Dict[str, Any], 오_이름: str, 오: Dict[str, Any]) -> None:
    ...
    print("\n  → 읽는 순서: '괘가 다른 쌍 공유'와 '전역 상투구'를 먼저 본다. 둘 다")
    print("    내려갔으면 좋아진 것이다.")
    print("    '대조군과의 차이'는 **대조군이 그대로일 때만** 뜻이 있다. 상투구가 전반적으로")
    print("    줄면 대조군도 같이 내려가 차이가 작아지므로, 그 줄만 보면 개선을 악화로 읽는다.")
    print("    실제로 그런 일이 있었다 — 다른괘 8.4%→8.2%로 그대로인데 대조군이")
    print("    28.0%→24.4%로 내려가 차이가 19.5%→16.2%가 됐다.")
